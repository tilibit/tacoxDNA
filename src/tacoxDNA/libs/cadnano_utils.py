...
class StrandGenerator(object):
    def generate(
        ...
        Generate a strand of DNA.
            ...
            Note: Relevant argument(s) in parentheses.
        ...
        circular --- Generate closed circular DNA (default False)
            Limitations...
        ...
        Note: Minimum circular duplex is 18. Shorter circular strands disobey FENE.
        ...
        if sequence is None:
            ...
            base.Logger.log(
                "sequence is too short, adding %d random bases" % n, base.Logger.WARNING
            )

        if circular and bp < 30:
            # 30 is about the cut off for circular dsDNA. Anything shorter will probably clash.
            # oxDNA can relax down to 18.
            # 4 is about the cut off for circular ssDNA. Use dsDNA cutoff for safety.
            base.Logger.log(
                "sequence is too short! Proceed at your own risk", base.Logger.WARNING
            )

        option_use_helicity = True
        if circular and bp < 30 and not double:
            base.Logger.log(
                "sequence is too short! Generating ssDNA without imposed helicity",
                base.Logger.WARNING,
            )
            # Do not impose helicity to generate shorter circular ssDNA
        ...
        # Circular strands require a continuous deformation of the ideal helical pitch
        if circular:
            ...
            angle = 2.0 * np.pi / float(bp)
            # Radius of torus
            radius = base.FENE_R0_OXDNA / math.sqrt(2.0 * (1.0 - math.cos(angle)))

        if circular and option_use_helicity:
            ...
            for i in range(bp):
                ...
                v_torus = v1 * base.BASE_BASE * math.cos(
                    i * angle
                ...
                R = utils.get_rotation_matrix(
                    a3,
                    [
                        i * (round(bp // BP_PER_TURN) + DELTA_LK) / float(bp) * 360,
                        DEGREES,
                    ],
                )
                ...
                a1 = np.dot(R, a1)
                ns1.add_nucleotide(
                    base.Nucleotide(rb - base.CM_CENTER_DS * a1, a1, a3, sequence[i])
                )
            ns1.make_circular(check_join_len=True)
        elif circular and not option_use_helicity:
            for i in range(bp):
                ...
                ns1.add_nucleotide(
                    base.Nucleotide(rb, a1, np.array([0, 0, 1]), sequence[i])
                )
            ns1.make_circular(check_join_len=True)
        else:
        ...
        if double:
            ...
            if ds_start == 0 and ds_end == bp and circular:
    ...
    def generate_or_sq(
        ...
        if length_change and len(region_begin) != len(region_end):
            if (len(region_end) + 1) == len(region_begin):
                ...
                base.Logger.die(
                    "the lengths of begin (%d) and end (%d) arrays are unrecoverable mismatched"
                    % (len(region_begin), len(region_end))
                )
        ...
        if double:
            ...
            for i in range(bp):
                # create new nucleotide and save basepairs info on both sides
                paired_nuc = ns1._nucleotides[bp - i - 1]
                new_nuc = base.Nucleotide(
                    rb - base.CM_CENTER_DS * a1, a1, a3, sequence2[i], pair=paired_nuc
                )
                ...
                if i != bp - 1:
                    ...
                    if length_change:
                        for j in range(len(length_change)):
                            if (
                                bp - 2 - i >= region_begin[j]
                                and bp - 2 - i < region_end[j]
                            ):
                                if length_change[j]:
                                    rb += (
                                        a3
    ...
    def generate_double_offset(
        ...
        s1, s2 = self.generate(bp, None, start_pos, direction, False, True, 0.0)
    ...
    def generate_rw(self, sequence, start_pos=np.array([0.0, 0.0, 0.0])):
        """
...
class vhelix_vbase_to_nucleotide(object):
    # at the moment squares with skips in have entries in the dicts,
    # but with the nucleotide list empty (rather than having no entry)
    # - I'm not sure whether or not this is desirable. It's probably ok
    def __init__(self):
        self._scaf = {}
        self._stap = {}
        self.nuc_count = (
            0  # record the nucleotide count, updated only after a whole strand is added
        )
        self.strand_count = 0
    ...
    def add_scaf_strand(self, add_strand, reference, continue_join=False):
        ...
        for (vh, vb), [strand_ind, nuc] in reference._scaf.items():
            if strand_ind == add_strand:
                self.add_scaf(
                    vh, vb, self.strand_count, [x + self.nuc_count for x in nuc]
                )
                count += len(nuc)
    ...
    def add_strand(self, add_strand, reference, continue_join=False):
        if self.add_scaf_strand(
            add_strand, reference, continue_join
        ) and self.add_stap_strand(add_strand, reference, continue_join):
            return 1
        else:
            return 0
...
def complementary_sequence(sequence):
    complement = []
    for b in list(sequence):
        if b == EMPTY:
            complement.append(EMPTY)
        else:
            complement.append(convert_base(b, COMPLEMENTARY_BASE))
...
def check_input(virtual_strand):
    ...
    for sequence_type in ["scafSeq", "stapSeq"]:
        virtual_strand[sequence_type] = [
            unknown_type_check(sequence) for sequence in virtual_strand[sequence_type]
        ]

    # check sequence lengths (fixing legacy error)
    new_virtual_strand = {}
    new_virtual_strand["scafSeq"] = virtual_strand["scafSeq"][
        : len(virtual_strand["scaf"])
    ]
    new_virtual_strand["stapSeq"] = virtual_strand["stapSeq"][
        : len(virtual_strand["scaf"])
    ]
...
def process_sequence(new_virtual_strand):
    vs_sequence = combine_scaf_staples(
        new_virtual_strand["scafSeq"], new_virtual_strand["stapSeq"]
    )
